...
RUN git clone https://github.com/supabase/index_advisor && \
    cd index_advisor && \
    make && \
    make install  
...
services:
  openobserve:
    image: openobserve/openobserve:v0.90.0-rc3
    deploy:
      resources:
        limits:
          cpus: '2'
    ...
    restart: unless-stopped
    environment:
      ZO_ROOT_USER_EMAIL: "admin@bmyf.tech"
      ZO_ROOT_USER_PASSWORD: "wPULWsoF5@t#Qr"
    ports:
      - "5080:5080"
    volumes:
      - ${PWD}/data:/data
...
services:
  vector:
    ...
    volumes:
      # 1. 挂载配置文件到容器内
      - ${PWD}/conf/vector.yaml:/etc/vector/vector.yaml:ro
      # 2. (关键) 挂载宿主机的 Docker 套接字
      - /var/run/docker.sock:/var/run/docker.sock:ro
      # 3. (可选) 用于磁盘缓冲的持久化目录
      - ${PWD}/vector-data:/var/lib/vector
    environment:
      - VECTOR_ENV=prod-channel-user
    healthcheck:
      test: ["CMD", "vector", "list"]
      interval: 30s
      timeout: 10s
      retries: 3
```
...
transforms:
  ...
  parse_json_logs:
    ...
    source: |
      .processed_at = now()
      .environment = get_env_var!("VECTOR_ENV")  # 从环境变量获取环境标识

      # 尝试将 .message 解析为 JSON
  log_file_transform_reduce:
    inputs:
      - parse_json_logs
    type: reduce
...
sinks:
  ...
  console_sink:
    type: console
    inputs: ["log_file_transform_reduce"]
    encoding:
      codec: json
  openobserve_log_sink:
    ...
    inputs:
      - log_file_transform_reduce
    uri: "http://172.27.0.9:5080/api/default/prod-channer-server/_json"
    method: "post"
    auth:
      strategy: "basic"
      user: "admin@bmyf.tech"
      password: "wPULWsoF5@t#Qr"
    encoding:
      codec: json
    compression: "gzip"
    healthcheck:
